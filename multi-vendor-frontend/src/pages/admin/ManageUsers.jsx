    ...
    <div className="p-8 min-h-screen bg-gradient-to-br from-indigo-50 via-pink-50 to-purple-100">
      ...
      {loading ? <div>Loading...</div> : (
        <div className="overflow-x-auto">
          <table className="w-full border rounded-xl overflow-hidden bg-white shadow-lg min-w-[600px]">
            <thead>
            ...
            <tbody>
              {users.map(user => (
                <tr key={user._id} className="hover:bg-pink-50 transition">
                  <td className="p-2 border break-all text-xs md:text-sm">{user._id}</td>
                  <td className="p-2 border text-xs md:text-sm">{user.name}</td>
                  <td className="p-2 border text-xs md:text-sm">{user.email}</td>
                  <td className="p-2 border flex items-center gap-1 text-xs md:text-sm">{getRoleIcon(user.role)}{user.role.charAt(0).toUpperCase() + user.role.slice(1)}</td>
                  <td className="p-2 border flex flex-col md:flex-row gap-2 items-center text-xs md:text-sm">
                    {user.role === 'customer' && (
